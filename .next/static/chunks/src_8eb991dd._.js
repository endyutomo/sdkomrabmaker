                    ...
                    try {
                        ...
                        const timeoutPromise = new Promise({
                            "useSupabaseQuery.useEffect.fetchData": (_, reject)=>setTimeout({
                                    "useSupabaseQuery.useEffect.fetchData": ()=>reject(new Error('Query timeout - check RLS policies'))
                                }["useSupabaseQuery.useEffect.fetchData"], 10000)
                        ...
                        if (error) {
                            console.error("Supabase error for ".concat(table, ":"), error);
                            throw error;
                        }
                        setData(data);
                        setError(null);
    ...
    const fetchCollaborations = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "DashboardPage.useCallback[fetchCollaborations]": async ()=>{
            ...
            try {
                ...
                // Get collaborations for current user with timeout
                const { data: collabs, error } = await supabase.from('project_collaborators').select('*, projects(*)').eq('user_id', user.id);
                if (error) {
                    // If table doesn't exist or RLS error, just skip
                    console.warn('Collaboration fetch error (might be expected):', error.message);
                    setCollaborationProjects([]);
                    setIsCollabLoading(false);
                    return;
                }
                if (collabs) {
                    const projects = collabs.map({
...
_s(DashboardPage, "6Gz1rBHoSgnOcFftIsXfokmKkg8=", false, function() {
    return [
...
//# sourceMappingURL=src_8eb991dd._.js.map