/**
 * Shared prompt template for AI price suggestions.
 * Used by both server-side Genkit flow and client-side Puter AI.
 */
...
Instruksi:
1. DETEKSI BRAND & MODEL: Periksa apakah item mengandung brand premium. Jika nama item sangat umum (misal: "Smart TV 55 inch"), sarankan satu MODEL TYPE spesifik yang lazim di pasaran Indonesia (misal: "Samsung Crystal UHD UA55AU7000").
...
export const createPriceSuggestionPrompt = (itemName: string, itemType: string): string => {
  return `${PRICE_SUGGESTION_SYSTEM_PROMPT}
